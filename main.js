...
const kaydetButton = document.querySelector(".column-card button");
fullName.addEventListener("input", (event) => {
	const capName = event.target.value.toUpperCase();

	event.target.value = capName;
...
*/
const formSubmit = kaydetButton.addEventListener("click", () => {
	let submitResult = document.getElementById("submitResult");
	submitResult.textContent = `${fullName.value} başarı ile kaydedildi.`;
	fullName.value = "";
	kaydetButton.disabled = true
